...
module.exports = {
  ...
  async execute(interaction) {
    ...
    if (interaction.channelId !== config.Premium_Gen_Channel) {
      const wrongChannelEmbed = new MessageEmbed()
        .setColor(config.color.red)
        .setTitle("Wrong command usage!")
        .setDescription(
          `You can only use the \`/premium\` command in <#${config.Premium_Gen_Channel}> channel!`,
        )
        .setFooter(
    ...
    if (generated.has(member.id)) {
      ...
      let timeRemaining =
        (config.Premium_Gen_Cooldown * 1000 - timeElapsed) / 1000;
    ...
    fs.readFile(filePath, "utf-8", (error, data) => {
      ...
      fs.writeFile(filePath, updatedData, (writeError) => {
        ...
        }, config.Premium_Gen_Cooldown * 1000);
      });
    });
  },
};
