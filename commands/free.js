...
module.exports = {
  ...
  async execute(interaction) {
    ...
    if (interaction.channelId !== config.Free_Gen_Channel) {
      const wrongChannelEmbed = new MessageEmbed()
        .setColor(config.color.red)
        .setTitle("**Wrong command usage!**")
        .setDescription(
          `You can only use the \`/free\` command in <#${config.Free_Gen_Channel}> channel!`,
        )
        .setFooter(
    ...
    if (generated.has(member.id)) {
      ...
      let timeRemaining =
        (config.Free_Gen_Cooldown * 1000 - timeElapsed) / 1000;
