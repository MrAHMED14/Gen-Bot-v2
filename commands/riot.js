...
module.exports = {
  ...
  async execute(interaction) {
    ...
    if (interaction.channelId !== config.Riot_Gen_Channel) {
      const wrongChannelEmbed = new MessageEmbed()
        .setColor(config.color.red)
        .setTitle("Wrong command usage!")
        .setDescription(
          `You can only use the \`/riot\` command in <#${config.Riot_Gen_Channel}> channel!`,
        )
        .setFooter(
    ...
    if (generated.has(member.id)) {
      const array = Array.from(generated);
      const time = array[1];
      let now = new Date().getTime();
      let timeElapsed = now - time;
      let timeRemaining =
        (config.Riot_Gen_Cooldown * 1000 - timeElapsed) / 1000;
      ...
      const cooldownEmbed = new MessageEmbed()
        ...
        .setTitle("Cooldown!")
        .setDescription(
          `Please wait **${minutes}** min before executing that command again!`,
        )
        .setFooter(
          interaction.user.tag,
          interaction.user.displayAvatarURL({ dynamic: true, size: 64 }),
    ...
    fs.readFile(filePath, "utf-8", (error, data) => {
      ...
      fs.writeFile(filePath, updatedData, (writeError) => {
        ...
        const embedMessage = new MessageEmbed()
          ...
          .setDescription(
            "🙏 Thank you so much for being a KING member! \n 🌟 Your support means the world to us! 💖😊",
          )
          .addField(
            "Service",
        ...
        setTimeout(() => {
          ...
          generated.delete(timeStamp);
        }, config.Riot_Gen_Cooldown * 1000);
      });
    });
  },
};
